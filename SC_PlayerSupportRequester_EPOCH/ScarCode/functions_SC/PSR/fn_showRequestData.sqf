...
if (not(isNull(findDisplay _dsp))) then
{
	if (count SC_tasks > 0) then
	{
		...
		_tsk = SC_tasks select _ind; // _tsk is the task
		_unt = _tsk select 6; // Unit
		...
		_txt = parseText format ["<t size='1.5'>#%1</t> from <t color='#ffc804'>%2</t> - Assigned: <t size='0.85' color='#ffc804'>%3</t><br />Location: <t color='#ffc804'>%4</t> - Type: <t color='#ffc804'>%5</t><br />Comment: <t color='#ffc804'>%6</t>", _tsk select 5, _tsk select 0, _unt, _tsk select 1, _tsk select 2, _tsk select 3];
